...
class World {
    ...
    async initialize() {
        ...
        document.getElementById('next').addEventListener("click", this.onClick.bind(this)) 
        document.getElementById('previous').addEventListener("click", this.onClick.bind(this))
        this.animate()
    }
    ...
    async getCommercials() {
        const response = await fetch('commercials.json')
    ...
    setText() {
        ...
        const currentReturn = (portfolioAmount - (100 * (this.plane_current_index + 1))).toFixed(2)
        console.log(currentCommercial)
        document.querySelector('#cumulativeReturn').textContent = 'Cumulative: $' + currentReturn
